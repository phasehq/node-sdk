...
describe("Crypto Utils Tests", () => {
  ...
  test("clientSessionKeys generates keys of correct length", async () => {
    ...
    const clientKeys = await clientSessionKeys(
      clientKeyPair,
  ...
  test("serverSessionKeys generates keys of correct length", async () => {
    const serverKeyPair = await randomKeyPair();
    const clientKeyPair = await randomKeyPair();
    const serverKeys = await serverSessionKeys(
      serverKeyPair,
...
describe("Asymmetric Encryption and Decryption Tests", () => {
  test("encryptAsymmetric and decryptAsymmetric return original plaintext", async () => {
    ...
    const decryptedData = await decryptAsymmetric(
      encryptedData,
      privateKeyHex,
      publicKeyHex
    );
  ...
  const knownHashes = [
    {
      ...
      salt: "world",
      expectedHash:
        "38010cfe3a8e684cb17e6d049525e71d4e9dc3be173fc05bf5c5ca1c7e7c25e7",
    },
    {
      inputStr: "another test",
      salt: "another salt",
      expectedHash:
        "5afad949edcfb22bd24baeed4e75b0aeca41731b8dff78f989a5a4c0564f211f",
    },
  ];
